...
- **Logging out deletes the local keyring.** `logout()` wipes this device's private keys along with the session (`clearKeyring` in `keyStorage.js`) — they don't just sit in `localStorage` between sessions. That means every login requires re-importing `keys.txt` (or generating a fresh pool) to read anything again; there's no "stay logged in with keys cached" state.
